...
function makeOne() {
	countOfForms = document.forms.length;
	paragraph.textContent = `Количество форм на странице: ${countOfForms}`;
}
...
function makeTwo() {
	nameOfFirstForm = document.forms[0].getAttribute('name');
	paragraphTwo.textContent = `Значение атрибута name первой формы на странице - ${nameOfFirstForm}`;
}
...
function makeThree() {
	nameOfLastForm = document.forms[document.forms.length - 1].getAttribute('name');
	paragraphThree.textContent = `Значение атрибута name последней формы на странице  - ${nameOfLastForm}`;
}
...
function makeFive() {
	countElemFormThree = document.forms.formThree.elements.length;
	paragraphFive.textContent = `Количество элементов в третьей форме - ${countElemFormThree}`;
}
...
function makeSix() {
	const countElemFormTwo = document.forms.formTwo.elements.length
	paragraphSix.textContent = `Количество элементов в третьей форме - ${countElemFormTwo}`;
}
...
function makeSeven() {
	const collectionOfForms = document.forms;
    const secondForm = collectionOfForms[1];
	const formElements = secondForm.elements.name;
	elementsList = '';
    for (let i = 0; i < document.forms[1].elements.length; i++){
		elementsList += `- ${secondForm.elements[i].name}`;
	}
...
function makeEight() {
	...
	const namesOfFirstForm = [];
	for (let i = 0; i < firstForm.elements.length; i++){
		namesOfFirstForm.push(firstForm.elements[i].name);
	}
	paragraphEight.textContent = namesOfFirstForm;
}
...
function makeNine() {
	...
	const namesOfThirdForm = [];
	for (let i = 0; i < thirdForm.elements.length; i++){
		namesOfThirdForm.push(thirdForm.elements[i].name);
	}
	paragraphNine.textContent = namesOfThirdForm;
}
...
function makeTen() {
	const valueOfRadioFourthForm = document.forms[3].elements.fourthName.value;
	paragraphTen.textContent = valueOfRadioFourthForm;
}
...
function makeEleven() {
	const firstForm = document.forms[0];
    const selectOfFirstForm = firstForm.elements.firstSelect;
	optionsValues = '';
	for (let i = 0; i < selectOfFirstForm.length; i++){
		optionsValues += `${selectOfFirstForm[i].value} `;
	}
...
function makeTwelve() {
	...
	idOfCheckbox = `${checkboxOne.id}, ${checkboxTwo.id}, ${checkboxThree.id}`;
	paragraphTwelve.textContent = idOfCheckbox;
}
...
const radio = fourthForm.elements.fourthName;
...
document.querySelector('.b-13').addEventListener('click', checkButton); //работает вне зависимости от задания 23
...
function checkOption() {
	const firstForm = document.forms[0];
	const firstFormSelect = firstForm.elements.firstSelect;
	if (firstFormSelect.selectedIndex == 0) {
		paragraphFourteen.textContent = 'Выбран первый вариант';
	} else if (firstFormSelect.selectedIndex == 1) {
...
function makeFifteen() {
	const firstForm = document.forms[0];
	const firstFormSelect = firstForm.elements.firstSelect;
	firstFormSelect.selectedIndex = 0;   
}
...
formOne.addEventListener('submit', function (event) {
	...
	const firstEmail = formOne.firstEmail;
	let errorMessage = document.getElementById('errorMessage');

	if (firstName.value === '') {
		errorMessage.textContent = 'Не заполнено имя.';
		return;
	}

	if (firstEmail.value === '') {
		errorMessage.textContent = 'Не заполнен email.';
		return;
	}

	if (firstEmail.value !== '' && firstName.value !== '') {
...
const emailInput = formOne.firstEmail;
...
emailInput.oninput = function () {
	...
	if (emailRegExp.test(emailValue)) {
		...
		errorMessage.textContent = 'Введите email в формате *@*.*';
    }
};
...
document.querySelector('.b-21').onclick = function (event) {
	...
	if (!isChecked) {
		...
		event.preventDefault();
		document.getElementById('result21').textContent = 'Необходимо выбрать хотя бы один вариант.';
	}
};
...
const fourthButton = fourthForm.elements.fourthButton;
const radioBox = fourthForm.elements.fourthName;
fourthButton.setAttribute('disabled', 'disabled');
radioBox.addEventListener('change', function() {
	fourthButton.disabled = !radioBox.checked;
});
...
const paragraph27 = document.getElementById('result27');

formTwoInputs.forEach(function (input) {
	input.addEventListener('input', function () {
		paragraph27.textContent = "Изменение внесено";
	});
});
...
const paragraph28 = document.getElementById('result28');

selectFormThree.addEventListener('change', function () {
	paragraph28.textContent = "Опция выбрана";
});
